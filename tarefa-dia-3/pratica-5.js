//Calculando as vendas com e sem gorjeta
...
//1: Qual o valor total de vendas sem gorjeta?

let totalDeVendasSemGorjeta = 0
let i = 0

while (i < receita.length) {
    totalDeVendasSemGorjeta += receita[i].valor
    i++
}

console.log(`O valor total de vendas sem gorjeta R$: ${totalDeVendasSemGorjeta} `)

//2: Qual o valor total de vendas incluindo a gorjeta? 

let valorTotalVendasComGorjeta = 0
i = 0

while (i < receita.length) {
    valorTotalVendasComGorjeta += receita[i].valor + receita[i].gorjeta
    i++
}

console.log("Valor total de vendas com gorjeta R$: ", valorTotalVendasComGorjeta)

//PROBLEMA 3: Quantas vendas foram abaixo de 50 reais? 

let vendasAbaixoDe50Reais = 0
i = 0

while (i < receita.length) {
    const valorTotalDeUmaVenda = receita[i].valor + receita[i].gorjeta
    if (valorTotalDeUmaVenda < 50)
        vendasAbaixoDe50Reais++
}

console.log("Número de vendas abaixo de 50 reais $: ", vendasAbaixoDe50Reais)
